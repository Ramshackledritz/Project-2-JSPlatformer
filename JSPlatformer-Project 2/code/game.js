...
Rain.prototype.type = "rain";

function Life(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2,0.1));
  this.size = new Vector (1,0);
  this.wobble = Math.random() * Math.PI * 2;
}
...
Level.prototype.actorAt = function(actor) {
  ...
  Rain.prototype.act = function(step, level) {
    var newPos = this.pos.plus(this.speed.times(step));
    if(!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
    else if (this.repeatPos)
  ...
  Life.prototype.act = function(step) {
    this.wobble += step * lifeSpeed;
    var wobblePos = Math.sin(this.wobble) * lifeDist;
  ...
  Level.prototype.playerTouched = function(type, actor) {
    ...
    // Player loses
    if (type == "lava" || type == "rain" && this.status == null) {
      this.status = "lost";
      this.finishDelay = 1;
    ...
    // If there aren't any coins or lives left, player wins
    if (!this.actors.some(function(actor) {
      return actor.type === "coin" || actor.type == "life"
    })) {
      this.status = "won";
      this.finishDelay = 1;
    }
